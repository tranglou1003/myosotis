    ...
    def __init__(self, pt_path='checkpoints/yolov5m-face.pt', confThreshold=0.5, nmsThreshold=0.45, device='cuda'):
        ...
        if device == 'cuda' and torch.cuda.is_available():
            try:
                ...
                test_tensor = test_tensor.float() / 255
                self.test_device = torch.device(device)
            except RuntimeError as e:
                if "no kernel image is available" in str(e):
                    print(f"CUDA compatibility issue detected: {e}")
                    print("Falling back to CPU for face detection...")
                    self.test_device = torch.device("cpu")
                else:
                    raise e
        else:
            self.test_device = torch.device("cpu" if not torch.cuda.is_available() else device)
